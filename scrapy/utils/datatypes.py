...
class CaselessDict(dict):
    ...
    def __init__(self, seq=None):
        ...
        #首先调用dict的初始化方法
        if seq:
        #如果seq不为空，那么update(seq),这个函数做了什么？往下研究看看。
            self.update(seq)
            #将seq中的键值加入到当前字典，键变为小写。注意实现。
    ...
    def update(self, seq):
    #这个函数就是将seq中的键小写，之后然后加入到当前字典中。
    #因为是大小写不敏感的字典。

        seq = seq.items() if isinstance(seq, dict) else seq
	#seq.items()将字典中的键值组成元组列表返回。
	#如果不是字典，则直接引用。
        #{'a':'a','b':'b'}
        #[('a', 'a'), ('b', 'b')]

        iseq = ((self.normkey(k), self.normvalue(v)) for k, v in seq)
	#normkey,mormvalue进行了什么操作呢？
        #将key小写，直接返回value
